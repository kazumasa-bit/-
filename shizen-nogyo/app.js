...
(function () {
  ...
  /* ───────── タブ3：マイ畑 ───────── */
  /* 種の保管のしかた（作物によらない共通の基本）
     作物ごとの「乾かして保存」は SEED[id].dry、寿命は SEED_LIFE にある。
     ここはその手前の、置き場所と乾かし方の話。 */
  function seedKeepPanel_() {
    function box(t, b) {
      return '<div style="margin-top:12px"><div style="font-weight:700;color:var(--ink)">' + t + '</div>' +
        '<div style="margin-top:2px">' + b + '</div></div>';
    }
    var h = '<details class="card" style="margin-top:12px">';
    h += '<summary style="cursor:pointer;padding:13px 15px;font-weight:700">🫙 種の保管のしかた</summary>';
    h += '<div class="card-pad" style="padding-top:0;font-size:13px;line-height:1.95;color:var(--ink-sub)">';

    h += '<div style="font-weight:700;color:var(--ink)">ひとことで言うと</div>' +
      '<div><b>よく乾かして、密閉して、冷蔵庫。</b>種を早く弱らせるのは、順に' +
      '<b>湿気 → 温度 → 光</b>。この3つを断つだけで、寿命は倍ちがいます。</div>';

    h += box('目安になる数字',
      '種屋でよく使われる目安が2つあります。<br>' +
      '・<b>置き場所の温度（℉）＋ 湿度（％）が 100 以下</b>なら合格。' +
      '10℃（＝50℉）なら湿度50％まで。冷蔵庫（5℃＝41℉）なら59％まで許せる計算です<br>' +
      '・<b>温度が5℃下がるごとに、種の寿命はおよそ倍</b>。乾き具合（含水率）が1％下がっても、およそ倍<br>' +
      '<span style="color:var(--ink-sub2)">どちらもざっくりした目安で、常識的な範囲（含水率5〜14％あたり）での話です。</span>');

    h += box('① 乾かす',
      '袋のままではまだ湿っています。<b>シリカゲルか、乾煎りして冷ました生米</b>と一緒に' +
      '密閉容器へ入れて1〜2週間。<br>' +
      '乾けたかの見分け方：<b>豆や大きい種は、指で折るとポキッと折れる</b>。' +
      'ぐにゃりと曲がるならまだ水気があります。');

    h += box('② 密閉する',
      '<b>ガラス瓶（パッキンつき）か、チャック袋を二重に。</b>紙の種袋のままは、まわりの湿気を吸います。' +
      '容器の中に乾燥剤をひとつ入れておく。<br>' +
      '袋には<b>作物名・品種・採った（買った）年・どの畑か</b>を書く。年が分からない種は、' +
      'この在庫箱に入れられません。');

    h += box('③ 冷やす・暗くする',
      '<b>冷蔵庫が家庭では一番。</b>野菜室でも冷蔵室でもかまいません。' +
      '冷凍もできますが、<b>完全に乾いている種だけ</b>（水気があると凍って中が壊れます）。<br>' +
      '冷蔵庫が使えないなら、家でいちばん涼しくて乾いた所（北側の押し入れなど）。' +
      '<b>台所・物置・車の中はやめる</b>——温度が上下して、そのたび湿気を吸います。');

    h += box('⚠ いちばんやりがちな失敗',
      '<b>冷蔵庫から出して、すぐ開ける。</b>冷えた種に空気の水滴がついて、一気に湿ります。' +
      '<b>容器のまま室温に戻してから開ける</b>（30分〜1時間）。使う分だけ取ったら、すぐ閉めて戻す。');

    h += box('④ 迷ったら発芽試験',
      '濡らしたキッチンペーパーに<b>10粒</b>包んで、袋に入れて暖かい所へ。' +
      '数日〜10日で<b>8粒出れば十分</b>、5粒なら厚めにまく、2〜3粒なら諦める。' +
      'まく直前ではなく、<b>まく2週間前</b>にやると打つ手があります。');

    h += box('寿命は作物でぜんぜん違う',
      'ネギ類（玉ねぎ・長ねぎ・にら）や人参は<b>1〜2年</b>で落ちます。' +
      'いっぽうトマト・ナス・きゅうり・かぼちゃは<b>4〜5年</b>もつ。' +
      '<b>この在庫箱は作物ごとの寿命で判定しています</b>ので、' +
      '上で登録すれば「あと何年まけるか」が出ます。');

    h += '<div style="margin-top:12px;color:var(--ink-sub2)">' +
      '※ 自分で採った種は、交雑にも注意。作付けタブで作物を開くと' +
      '「種を採る・まく準備」に、その作物の採りどき・乾かし方・交雑の注意が入っています。</div>';
